...
RUN apt-get update && apt-get install -y --no-install-recommends \
        build-essential software-properties-common autoconf automake libtool \
        pkg-config ca-certificates wget git curl libjpeg-dev libpng-dev \
        language-pack-en locales locales-all \
        python3 python3-py python3-dev python3-pip python3-numpy python3-pytest \
        python3-setuptools libprotobuf-dev protobuf-compiler zlib1g-dev \
        swig vim gdb valgrind libsm6 libxext6 libxrender-dev \
        cmake unzip sudo
